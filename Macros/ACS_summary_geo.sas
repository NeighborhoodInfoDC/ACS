...
%macro ACS_summary_geo( geo, source_geo );
  ...
  %note_mput( macro=ACS_summary_geo, msg=Creating &out_ds.... )
  
  %** Start of code for block group data **;
  ...
  ** Create summary data set **;
  ...
  %if ( &geo_name = GEO2000 and %upcase( &source_geo_var ) = GEO2000 ) or 
    ...
    ** Census tracts or counties source to same geo and year: just recopy selected vars **;
  ...
  %else %do;
    ...
    %** Check for non-existent weighting file; 
    %if %length( &geo_wt_file ) = 0 %then %do;
      %warn_mput( macro=ACS_summary_geo, msg=No weighting file. &out_ds will not be created. )
      %goto exit;
    %end;
    ...
    ** Transform data from source geography (&source_geo_var) to target geography (&geo_var) **;
    ...
    %else %do;
      ...
      %** Summary is not council districts **;
