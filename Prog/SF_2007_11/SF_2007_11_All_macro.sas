/**************************************************************************
 Program:  SF_2007_11_All_macro.sas
 Library:  ACS
 ...
 Description:  Adapted US Census SAS code for reading in ACS 5-year
 summary file data, 2007-11.
 
 NOTE: This is an included macro file, not an autocall library.

 Modifications:
**************************************************************************/

/*rootdir is a directory (that must exist) to store the generated SAS code				 */
/***PT*** FOLDER NAME MUST END WITH A \ ***/
%global rootdir;
%let rootdir = D:\DCData\Libraries\ACS\Prog\SF_2007_11\Code\;
...
data work.&geography;
  ...
  INFILE "&raw_folder\&geography..txt" MISSOVER TRUNCOVER LRECL=1500;
...
data _null_;
  ...
  /*Save code to FILE statement below*/
	...
	if _n_ =1 then 		 do;
		...
		put "		   LOGRECNO $7;";
		put " ";
		put "INFILE ""\&raw_folder\&type.20115&geo.&seq.000.txt"" DSD TRUNCOVER DELIMITER =',' LRECL=3000;";
		put " ";
