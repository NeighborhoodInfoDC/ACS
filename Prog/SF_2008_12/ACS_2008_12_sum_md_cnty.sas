/**************************************************************************
 Program:  ACS_2008_12_sum_md_cnty.sas
 Library:  ACS
 ...
 Description:  Create summary file from ACS 5-year data: 2008-12.
 
 Modifications:
  01/21/14 PAT  Updated for new SAS1 server (not tested).
  01/31/14 MSW	Adapted from ACS_2007_11_sum_all.sas
  03/03/14 PAT  Adapted from ACS_2008_12_sum_all.sas
                Added OCC0 vars.
**************************************************************************/
...
%global year year_lbl year_dollar stab;
...
%macro Summary_geo( geo, source_geo );
  ...
  %** Only supports COUNTY level **;
  %let geo_name = %upcase( &geo );
  %let geo_wt_file = ;
  %let geo_var = ucounty;
  %let geo_suffix = _cnty;
  %let geo_label = Full county ID: ssccc;
  ...
  %else %do;
    ...
    %let count_vars = 
           ...
           PopWithRace: PopBlack: PopWhite: PopHisp: PopAsian: PopNative: PopOther: PopMulti: 
           PopPoor: PopInCivLaborForce: PopCivilian: PopUnemployed: PopEmployed:
  ...
  %if ( &geo_name = GEO2000 and %upcase( &source_geo_var ) = GEO2000 ) or 
    ...
    ** Census tracts from census tract source: just recopy selected vars **;
    ...
    data ACS_r.&source_ds_work&geo_suffix (label="ACS summary, &year_lbl, &source_geo_label source, %upcase(&stab), &geo_label");
  ...
  %else %do;
    ...
    *OPTIONS MPRINT SYMBOLGEN MLOGIC;
    ...
    proc summary data=&source_ds_work nway;
      class &geo_var;
      var &count_vars;
      output 
        sum=
        out=ACS_r.&source_ds_work&geo_suffix 
              (drop=_type_ _freq_
               label="ACS summary, &year_lbl, &source_geo_label source, %upcase(&stab), &geo_label")
      ;
    run;
  
  %end;  

  proc datasets library=ACS_r memtype=(data) nolist;
...
%macro Summary_geo_source( source_geo );
  ...
  %if %upcase( &source_geo ) = BG00 %then %do;
     ...
     %let source_ds = Acs_sf_&year._&stab._bg00;
  ...
  %else %if %upcase( &source_geo ) = TR00 %then %do;
     ...
     %let source_ds = Acs_sf_&year._&stab._tr00;
  ...
  %else %if %upcase( &source_geo ) = BG10 %then %do;
     ...
     %let source_ds = Acs_sf_&year._&stab._bg10;
     %let source_geo_label = Block group;
  %end;
  %else %if %upcase( &source_geo ) = TR10 %then %do;
     ...
     %let source_ds = Acs_sf_&year._&stab._tr10;
     %let source_geo_label = Census tract;
  %end;
  %else %do;
  ...
  %let source_ds_work = ACS_&year._sum_&stab.&source_geo_suffix;
  ...
  ** Create new variables for summarizing **;
  ...
  data &source_ds_work;
    ...
    ** Add county varible **;
    ...
    label ucounty = "Full county ID: ssccc";
    ...
    ** Population by race/ethnicity **;
    ...
    %if %upcase( &source_geo ) = TR00 or %upcase( &source_geo ) = TR10 %then %do;
      ...
      ** Occupation **;
      ...
      OCC011 = sum( C24010e5, C24010e41 );
      OCC013 = sum( C24010e6, C24010e42 );
      OCC015 = sum( C24010e8, C24010e44 );
      OCC017 = sum( C24010e9, C24010e45 );
      OCC019 = sum( C24010e10, C24010e46 );
      OCC021 = sum( C24010e12, C24010e48 );
      OCC023 = sum( C24010e13, C24010e49 );
      OCC025 = sum( C24010e14, C24010e50 );
      OCC027 = sum( C24010e15, C24010e51 );
      OCC029 = sum( C24010e16, C24010e52 );
      OCC031 = sum( C24010e20, C24010e56 );
      OCC033 = sum( C24010e21, C24010e57 );
      OCC035 = sum( C24010e24, C24010e60 );
      OCC037 = sum( C24010e25, C24010e61 );
      OCC039 = sum( C24010e26, C24010e62 );
      OCC041 = sum( C24010e28, C24010e64 );
      OCC043 = sum( C24010e29, C24010e65 );
      OCC045 = sum( C24010e31, C24010e67 );
      OCC047 = sum( C24010e32, C24010e68 );
      OCC049 = sum( C24010e33, C24010e69 );
      OCC051 = sum( C24010e35, C24010e71 );
      OCC053 = sum( C24010e36, C24010e72, C24010e37, C24010e73 );

      PopEmployedOccConstruct_&year = sum(OCC047, OCC049);
      PopEmployedOccManagement_&year = sum(OCC011, OCC013, OCC015, OCC017, OCC019, 
                                           OCC021, OCC023, OCC025, OCC027, OCC029 );
      PopEmployedOccProduction_&year = sum(OCC051, OCC053);
      PopEmployedOccSales_&year = sum(OCC041, OCC043);
      PopEmployedOccService_&year = sum(OCC031, OCC033, OCC035, OCC037, OCC039);
      PopEmployedOccFarm_&year = sum( OCC045 );
      
      PopEmployedOccTotal_&year = sum( of OCC0: );
