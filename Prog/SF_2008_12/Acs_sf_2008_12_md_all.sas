/**************************************************************************
 Program:  Acs_sf_2008_12_md_all.sas
 Library:  ACS
 ...
 B19127	0063	AGGREGATE FAMILY INCOME IN THE PAST 12 MONTHS (IN 2011 INFLATION-ADJUSTED DOLLARS)
 B19131	0064	FAMILY TYPE BY PRESENCE OF OWN CHILDREN UNDER 18 YEARS BY FAMILY INCOME IN THE PAST 12 MONTHS (IN 2009 INFLATION-ADJUSTED DOLLARS)
 B23001	0075	SEX BY AGE BY EMPLOYMENT STATUS FOR THE POPULATION 16 YEARS AND OVER
 B25003	0099	TENURE
 ...
 Modifications:
  ...
  02/28/14 PAT Changed output file names from Acs_sf_2008_12_* to
               Acs_sf_2008_12_md_*. 
               Added B01002Ee/m: B25014Ee/m: B19013Ee/m: to _drop_list.
               Added 0063, 0064, 0075 to _seq_list.
               Corrected _years_dash value. 
**************************************************************************/
...
%let _state_fips = 24;           /** State FIPS code **/
%let _state_ab   = md;           /** State abbreviation **/
...
%let _seq_list = 0001 0002 0003 0004 0005 0009 0013 0018 0036 0037 0043 0044 0047 0056 0057 0058
                 0061 0062 0063 0064 0072 0075 0078 0079 0081 0099 0101 0102 0103 0104 0105 0106 0107;

%** Tables to read **;

%let _table_list = B00001 B00002 B01001 B01002 B01003 B02001 B03002 B05002 B06002 B07012
				   ...
				   B19001 B19013 B19050 B19056 B19057 B19058 B19059 B19127 B19131
...
%let _drop_list = B01001Ee: B01002Ee: B11001Ee: B19001Ee: B19013Ee: B25003Ee: B25014Ee: C24010Ee:
...
%macro Compile_ACS( geo );
  ...
  **** Combine data table & margin of error files ****;
  ...
  data Acs_sf_&_years._&_state_ab._&geo_suffix;
    ...
    ** Create standard geography variable **;
    ...
    %** If DC then set geo format and check geo ID values, otherwise skip. **;
    
    %if &_state_fips = 11 %then %do;
  ...
  ** Drop unneeded table cells **;
  ...
  data ACS_r.Acs_sf_&_years._&_state_ab._&geo_suffix (label="ACS SF 5-year tables, &_years_dash, %upcase(&_state_ab.), &geo_label");

    set Acs_sf_&_years._&_state_ab._&geo_suffix;
  ...
  proc sort data=ACS_r.Acs_sf_&_years._&_state_ab._&geo_suffix;
    by &geo_var;
  run;

  %File_info( data=ACS_r.Acs_sf_&_years._&_state_ab._&geo_suffix, printobs=0, freqvars=sumlevel )
