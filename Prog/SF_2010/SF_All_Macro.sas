...
LIBNAME stubs '/tab4/sumfile/prod/2010/docs';
LIBNAME sas '/prt03/kck/sumfile/code_JL/';
************************************************************************************
...
data work.&geography;
  ...
  INFILE "/tab4/sumfile/prod/2010/geo/&geography..txt" MISSOVER TRUNCOVER LRECL=500;
...
%let rootdir=/prt03/kck/sumfile/code_JL/;
...
data _null_;
  ...
  /*Save code to FILE statement below*/
	...
	if _n_ =1 then 		 do;
   		put "TITLE ""&type.20101&geo.&seq.000"";";
		...
		put "		   LOGRECNO $7;";
		put " ";
		put "INFILE '/tab4/sumfile/prod/2010/data/&type.20101&geo.&seq.000.txt' DSD TRUNCOVER DELIMITER =',' LRECL=3000;";
		put " ";
...
%do x=1 %to 179 ;
   ...
   * data sas.SF&seq&geo;
  	*  merge  g20101&geo(IN=g) SFe&seq&geo(IN=x) SFm&seq&geo(IN=y);
   	*	by logrecno;
 *	run;
%end;
